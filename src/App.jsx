  ...
  const toggleTimer = async (id) => {
    try {
      const newCustomers = customers.map(customer => {
        if (customer.id === id) {
          const newIsRunning = !customer.isRunning
          // If pausing, calculate new expectedEndTime based on remaining time
          if (!newIsRunning && customer.expectedEndTime) {
            // Paused - lock the current remaining time
            const now = Date.now()
            const endTimeMs = new Date(customer.expectedEndTime).getTime()
            const remainingMs = Math.max(0, endTimeMs - now)
            const newEnd = new Date(Date.now() + remainingMs)
            return { 
              ...customer, 
          ...
          return { ...customer, isRunning: newIsRunning }
        }
        return customer
      })
      setCustomers(newCustomers)
